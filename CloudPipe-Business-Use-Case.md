...
| **Terraform for infrastructure**  | The whole environment is version-controlled code, with `terraform plan` giving a full reviewable diff _before_ anything changes. Rebuildable in minutes, no "click-ops" drift, and it's the most in-demand IaC skill. | Console clicking = undocumented, unrepeatable, impossible to code-review. CloudFormation = solid AWS-native alternative (an equivalent template is kept in `infrastructure/` for comparison) — the core difference is state: AWS manages it for you vs. Terraform's state file you own. |
...
6. **Test the failure path too:** temporarily break the `S3_BUCKET` secret, push, and confirm you get a red X and an email. Fix the secret and use "Re-run jobs" — no new commit needed, because the _code_ was never wrong. Config and code are separate failure domains.
...
The pattern: solve today's problem, know what tomorrow's upgrade is, don't build it prematurely.
